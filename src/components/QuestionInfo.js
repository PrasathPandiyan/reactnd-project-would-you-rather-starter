        ...
        return (
            <div className='tweet'>
               <div className='col-md-3'>
                  <img className="avatar" src={questionInfo.avatarUrl} />
               </div>
                                ...
                                <div className="heading-3"> Poll Results: </div>
                                    <div className='col-md-9 col-md-offset-2 content'>
                                        <div className="heading-3"> {question.optionOne.text}:</div>
                                            ...
                                            <div style={{textAlign: 'center'}}>
                                                {Math.trunc(question.optionOne.votes.length/ Object.keys(users).length * 100)}% people voted
                                            </div>
                                        </div>
                                    <div className='col-md-9 col-md-offset-2 content'>
                                        <div className="heading-3"> {question.optionTwo.text}:</div>
                                        <div style={{textAlign: 'center'}}> {question.optionTwo.votes.length} Votes</div>
                                        <div style={{textAlign: 'center'}}>
                                            {Math.trunc(question.optionTwo.votes.length/ Object.keys(users).length * 100)}% people voted</div>
                                    </div>
                             </div>
                            ) : (
                            <div>
                                <form onSubmit={this.onClick}>
                                    <div className='col-md-9 col-md-offset-3 content'>
                                        <input type='radio' name='pollingQues' value='optionOne' checked={this.state.value === 'optionOne' } onChange={this.onChange}/>
                                        {question.optionOne.text}
                                    </div>
                                    <div className='col-md-9 col-md-offset-3 content'>
                                        <input type='radio' name='pollingQues' value='optionTwo' checked={this.state.value === 'optionTwo' } onChange={this.onChange}/>
                                        {question.optionTwo.text}
                                    </div>
                                    <div className='btnHead'>
                                        <button onClick={this.onClick} className="btn"> Submit</button>
                                    </div>
                                </form>
                            </div>
                        )}
...
function mapStateToProps ({ authedUser, questions, users }, props) {
    ...
    const questionInfo = getQuestionInfo(users, question, authedUser);
